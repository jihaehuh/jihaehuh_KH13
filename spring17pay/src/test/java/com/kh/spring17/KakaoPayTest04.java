...
//		KakaoPayReadyRequestVO requestVO = new KakaoPayReadyRequestVO();
		...
		header.add("Authorization", "SECRET_KEY "+ kakaoPayProperties.getKey());  //key 뒤에 띄어쓰기 조심 
		header.add("Content-Type", "application/json");
		
		//바디 생성
		Map<String, String> body = new HashMap<>();
		body.put("cid",kakaoPayProperties.getCid());  //프로퍼티즈에서 값 불러오기 
		...
		body.put("quantity", "1");
		body.put("total_amount", String.valueOf(requestVO.getTotalAmount()));  //인트를 스트링으로 바꾸기 
...
//		Map response = template.postForObject(uri, entity, Map.class);
		...
		log.debug("거래번호(tid) ={}",responseVO.getTid());
