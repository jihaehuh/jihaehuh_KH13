...
public class Test06끝말잇기게임 {
	public static void main(String[] args) {
		...
		while(true) {
			...
			boolean isKoreanWord = input.matches("^[가-힣]{2,6}$"); //한글단어 2-6글자
			boolean isContain = history.contains(input); //반복되는지
			boolean isConnect = given.charAt(given.length()-1) == input.charAt(0); //마지막글자와 첫글자 이어지는지
